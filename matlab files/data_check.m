...
close all
clc
cd /home/robot/workspaces/ur5_mpc_ursim/data_0901
iter_AB_eval = 1;
iter_AB_train = 1;
iter_AB_test = 1;
iter_BA_eval = 1;
...
for i =1:files
    ...
    if len>10
        ...
        iter=1;
        for w = s:e
            if kkt_val_high(w)>0.01 && smallest_distance(w)>0.4
...
clc
for i =1:59
    filename = sprintf('data_BA_test_%i.csv',i);
    
    low = readtable(filename);
    low = table2array(low);
    [len,l] = size(low);
    human_sphere = low(:,7:48);

    joint_positions = low(:,1:6);
    low_MPC_solutions =  low(:,46:54);
    kkt_val_high = low(:,55);
    smallest_d = low(:,56);
